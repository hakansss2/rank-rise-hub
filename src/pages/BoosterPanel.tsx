...
import { Clock, CheckCircle, ArrowRight, MessageCircle } from 'lucide-react';
import Image from '@/components/ui/image';
import { format } from 'date-fns';

const BoosterPanel = () => {
  const { user, isAuthenticated, isBooster } = useAuth();
  const { getBoosterOrders, getAvailableOrders, setActiveOrder, claimOrder } = useOrder();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className="bg-valorant-black border border-valorant-gray/30 rounded-xl p-6 shadow-xl hover-scale">
            ...
            <div className="text-3xl font-bold">{boosterOrders.filter(o => o.status === 'in_progress').length}</div>
          </div>
          ...
          <div className="bg-valorant-black border border-valorant-gray/30 rounded-xl p-6 shadow-xl hover-scale">
            ...
            <div className="text-3xl font-bold">{boosterOrders.filter(o => o.status === 'completed').length}</div>
          </div>
        </div>
        ...
        <div className="bg-valorant-black border border-valorant-gray/30 rounded-xl p-6 shadow-xl mb-12">
          ...
          <Tabs defaultValue="available">
            <TabsList className="bg-valorant-gray/20 border border-valorant-gray/30">
              ...
              <TabsTrigger value="my-orders" className="data-[state=active]:bg-valorant-green data-[state=active]:text-white">
                Siparişlerim ({boosterOrders.length})
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="my-orders" className="pt-6">
              {boosterOrders.length === 0 ? (
                <div className="text-center py-12">
                  <div className="text-gray-400">Henüz hiç sipariş almadınız.</div>
                </div>
              ) : (
                <div className="space-y-4">
